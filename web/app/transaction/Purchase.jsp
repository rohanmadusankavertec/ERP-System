            ...
            xmlHttp.onreadystatechange = function ()
            {
                if (xmlHttp.readyState === 4 && xmlHttp.status === 200)
                {
                    ...
                    if (reply === "Success") {
                        ...
                        setTimeout("location.href = 'Purchase?action=ViewPurchase';", 1500);
                    } else {
                        sm_warning("Purchase is Not Added, Please Try again");
                    }
                }
            };
            xmlHttp.open("POST", "Purchase?action=SavePurchase&acc=" + acc + "&price=" + price + "&desc=" + desc + "&pacc=" + pacc + "&tdis=" + tdis + "&pamount=" + pamount + "&balance=" + balance+ "&cdtype=" + cdtype+ "&cdacc=" + cdacc, true);
            xmlHttp.send();
        }
    }
</script>
