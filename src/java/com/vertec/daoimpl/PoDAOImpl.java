...
public class PoDAOImpl{
    ...
    public Supplier getSupplier(int sid) {
        ...
        if (session != null) {
            try {
                ...
                System.out.println("SUPPLIER ID IS: "+sid);
 ...
 public String savePRN(PrnInfo prn) {
    ...
    public String savePO(PoInfo pi) {
        Session session = NewHibernateUtil.getSessionFactory().openSession();
        Transaction transaction = session.beginTransaction();
    ...
    public String savePOItems(PurchasingOrder pi) {
        Session session = NewHibernateUtil.getSessionFactory().openSession();
        Transaction transaction = session.beginTransaction();
    ...
    public List<PoInfo> getPOinfo(String type, String sup, String from, String to) {
        Session session = NewHibernateUtil.getSessionFactory().openSession();

        if (session != null) {
            try {
                Query query = session.getNamedQuery("PoInfo.findAll");
                List<PoInfo> poList = query.list();
                return poList;
    ...
    public String getItems1(String id) {
        ...
        if (session != null) {
            try {
                SQLQuery query = session.createSQLQuery("select p.product_name,po.qty,po.available_qty,po.price,po.total from purchasing_order po inner join product p on po.product_product_id=p.product_id where po_info_po_info_id='" + id + "'");
    ...
    public String ProductFromCategory(String id,Company company) {
        ...
        if (session != null) {
            try {
                SQLQuery query = session.createSQLQuery("SELECT product_id,product_code,product_name FROM product where product_category_id='"+id+"' AND company_id='"+company.getId()+"'");
