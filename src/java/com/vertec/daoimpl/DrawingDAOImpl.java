...
public class DrawingDAOImpl {
    ...
    public List<Account> loadAllOfAccount(Company com){
        System.out.println("1st...........");
        Session session = NewHibernateUtil.getSessionFactory().openSession();
        
        try {
            if(session != null){
            Query query = session.createQuery("SELECT a FROM Account a WHERE a.isValid=:valid AND a.subtypeId.name=:sid AND a.companyId=:com");
            query.setParameter("valid", true);
            query.setParameter("sid", "Drawing");
