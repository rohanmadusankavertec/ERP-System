 ...
 * @author vertec-r
 */
public class FixedAssetDAOImpl {
    public List<Account> loadAccountsToPurchase(Company company) {
        ...
        if (session != null) {
            try {
                ...
                query.setParameter("stype", "Fixed Asset");
    ...
    public List<Account> loadCreditorAccounts(Company company) {
        Session session = NewHibernateUtil.getSessionFactory().openSession();
        if (session != null) {
            try {
                Query query = session.createQuery("SELECT ac FROM Account ac WHERE ac.subtypeId.name=:stype AND ac.companyId=:company AND ac.isValid=:iv");
    ...
    public List<Account> loadDebtorAccounts(Company company) {
        Session session = NewHibernateUtil.getSessionFactory().openSession();
        if (session != null) {
            try {
                Query query = session.createQuery("SELECT ac FROM Account ac WHERE ac.subtypeId.name=:stype AND ac.companyId=:company AND ac.isValid=:iv");
                query.setParameter("company", company);
                query.setParameter("stype", "Debtors");
                query.setParameter("iv", true);
                List<Account> acList = query.list();
                return acList;
            } catch (Exception e) {
    ...
    public List<Account> loadDepreciationAccounts(Company company) {
        Session session = NewHibernateUtil.getSessionFactory().openSession();
        if (session != null) {
            try {
                Query query = session.createQuery("SELECT ac FROM Account ac WHERE ac.subtypeId.name=:stype AND ac.companyId=:company AND ac.isValid=:iv");
                query.setParameter("company", company);
                query.setParameter("stype", "Depreciation");
                query.setParameter("iv", true);
                List<Account> acList = query.list();
                return acList;
            } catch (Exception e) {
